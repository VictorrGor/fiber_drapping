...
{
	vertex** Q = new vertex *[2];
	for (int i = 0; i < 2; ++i)
	{
		Q[i] = new vertex[2];
	}
	Q[0][0].pos = vec3(-0.5, 0, -0.5);
	Q[0][1].pos = vec3(-0.5, 0, 0.5);
	Q[1][0].pos = vec3(0.5, 0, -0.5);
	Q[1][1].pos = vec3(0.5, 0, 0.5);
	...
	drapping_part(_rs, &si, 0, 0.5, true, 0, 0, false);
